...
function initGame() {
    // 初始化安全区域
    const safeArea = {
        top: Math.max(20, window.innerHeight * 0.05),
        bottom: window.innerHeight - Math.max(20, window.innerHeight * 0.05)
    }
    window.gameState = {
        safeArea: safeArea,
    ...
    function resizeCanvas() {
        ...
        // 更新安全区域
        gameState.safeArea.top = Math.max(20, height * 0.05);
        gameState.safeArea.bottom = gameHeight - Math.max(20, height * 0.05);
        gameState.topOffset = gameState.safeArea.top;
    ...
    class Fish {
      constructor(type) {
        ...
        const pondCenter = {
          x: canvas.width / 2,
          y: canvas.height / 2 + gameState.topOffset / 2
        }
    ...
    function gameLoop() {
      ...
      if (gameState.currentMode === 'pond') {
        ...
        ctx.strokeText(scoreText, 20, gameState.topOffset + 30)
        ctx.fillText(scoreText, 20, gameState.topOffset + 30)
      } else {
        // 特写模式
        ...
        ctx.strokeText(tipText1, canvas.width / 2, gameState.topOffset + 30)
        ctx.fillText(tipText1, canvas.width / 2, gameState.topOffset + 30)
        ctx.strokeText(tipText2, canvas.width / 2, gameState.topOffset + 60)
        ctx.fillText(tipText2, canvas.width / 2, gameState.topOffset + 60)

        // 绘制时间（移到提示文字下方）
        ctx.font = 'bold 24px Arial'
        ctx.textAlign = 'right'
        const timeText = `${Math.ceil(gameState.closeupTimer/1000)}秒`
        ctx.strokeText(timeText, canvas.width - 20, gameState.topOffset + 100) // 调整位置
        ctx.fillText(timeText, canvas.width - 20, gameState.topOffset + 100)
        ctx.textAlign = 'left'
    ...
    function drawWaterEffect() {
      const centerY = canvas.height / 2 + gameState.topOffset / 2
      const amplitude = 3
      const frequency = 30
      const waves = 3
